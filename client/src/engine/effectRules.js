...
function findEffectTargetByInstanceId(state, instanceId) {
  ...
  const handMatch = String(instanceId).match(/^(you|opponent)-hand-(\d+)$/);

  if (handMatch) {
    const side = handMatch[1];
    const handIndex = Number(handMatch[2]);
    const handCard = state?.[side]?.hand?.[handIndex];
...
export function validateEffectTarget(state, step, targetInstanceId) {
  ...
  if (!targetRef.card.rested) {
    return {
      valid: false,
      message: "That target is already active."
    };
  }
}
...
export function hasValidEffectTarget(state, step) {
  ...
  const possibleTargets = [
    ...
    ...(state.you?.hand || []).map((card, index) => ({
      ...card,
      instanceId: `you-hand-${index}`
    })),

    ...(state.opponent?.hand || []).map((card, index) => ({
      ...card,
...
export function applyEffectStep(state, step, targetInstanceId) {
  ...
  function playCardFromTrashToBoard(
    ...
    if (replaceIndex !== -1) {
      ...
      return {
        nextState,
        success: true,
        message: `${newCharacter.name || newCharacter.cardId} was played from trash ${entersRested ? "rested" : "active"
  ...
  targetRef.card.rested = false;

  return {
    nextState,
    success: true,
    message: `${targetRef.card.name || targetRef.card.cardId} was set as active.`
  };
}
  ...
  if (step.type === "hand_to_top_life") {
    ...
    if (targetRef.zone !== "hand") {
      return {
        nextState: state,
        success: false,
        message: "Choose a card from hand."
      };
    }

    const playerKey = step.player || targetRef.side || "you";
    const player = nextState[playerKey];
    ...
    if (targetRef.side !== playerKey) {
      return {
        nextState: state,
        success: false,
        message: "Choose your own hand card."
      };
    }

    const handIndex = targetRef.handIndex;

    if (handIndex == null || handIndex < 0 || handIndex >= player.hand.length) {
      return {
        nextState: state,
        success: false,
        message: "Invalid hand card."
      };
    }

    const [selectedCard] = player.hand.splice(handIndex, 1);
    ...
    player.life.unshift({
      ...lifeCard,
      instanceId: `${playerKey}-life-${Date.now()}`,
      attachedDon: [],
      rested: false,
      summoningSick: false,
      faceUp: true,
    ...
    return {
      nextState,
      success: true,
      message: `${selectedCard.name || selectedCard.cardId || "Card"} was placed on top of life face up.`
    };
  }
...
return {
  ...
  if (step.type === "move_attached_don") {
    ...
    return {
      nextState,
      success: true,
      message: `Moved ${movedDonIds.length} attached DON from ${sourceRef.card.name || sourceRef.card.cardId
  ...
  if (step.type === "attach_rested_don") {
    ...
    return {
      nextState,
      success: true,
      message: `Attached ${donIdsToAttach.length} rested DON to ${targetRef.card.name || targetRef.card.cardId
        }.`
    };
  }
